                    ...
                    if os.path.isdir(run_dir):
                        for run_item in os.listdir(run_dir):
                            if run_item.startswith('build_base_'):
                                ...
                                # 讀取 Makefile.spec 文件
                                makefile_spec_files = [f for f in os.listdir(build_dir) if f.startswith('Makefile') and f.endswith('.spec')]
                                if makefile_spec_files:
                                    makefile_spec_path = os.path.join(build_dir, makefile_spec_files[0])
                                else:
                                    makefile_spec_path = None
                                if os.path.exists(makefile_spec_path):
                                    ...
                                    if match:
                                        ...
                                        new_dir_path = os.path.join(benchmark_path + "/build/", new_dir_name)
                                        # 重命名目錄
